...
////
    ...
    Args(conf, this, errh).read_mp("MY_IP", _my_address).complete();
    return 0;
}
    ...
    for(Vector<TCPconnection*>::const_iterator i = connections.begin();i!=connections.end();i++){
        ...
        if(timer == &(conn->timer)){
            if(conn->state == ACTIVE_PENDING){
                ...
                click_chatter("Sending SYN_TCP(SEQ = %u) to %u.", syn_header->seqnum, syn_header->dstip);
                output(1).push(syn);
                ...
                Packet *syn = conn->window_unacked[0];
                struct TCPheader* syn_header = (struct TCPheader*) syn->data();
                click_chatter("Sending SYN_TCP ACK_TCP(SEQ = %u, ACK_TCP = %u) to %u.", syn_header->seqnum, syn_header->acknum, syn_header->dstip);
                output(1).push(syn);
                timer->schedule_after_msec(TIME_OUT);
            }
        }
...
//            conn->window_waiting.push_back(packet);
        ...
        click_chatter("Received packet %u from %u %u", header->seqnum, header->srcport, header->acknum);
        if(header->SYN_TCP && !header->ACK_TCP){
            ...
            if(header->acknum == conn->_seq+1){
                click_chatter("Receive SYN_TCP ACK_TCP, Connection to %u Established.", header->srcip);
                ...
                click_chatter("Sending ACK_TCP(SEQ = %u, ACK_TCP = %u) to %u.", ack_header->seqnum, ack_header->acknum, ack_header->dstip);
                output(1).push(ack);
